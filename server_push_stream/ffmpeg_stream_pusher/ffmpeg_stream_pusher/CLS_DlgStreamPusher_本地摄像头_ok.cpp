			...
			if (av_fifo_size(pStrctStreamInfo->m_pVideoFifo) >= size){
				...
				if (got_picture == 1){
					...
					pkt.dts = av_rescale_q_rnd(pkt.dts, pThis->m_pFmtVideoCtx->streams[0]->time_base,
						pThis->m_pFmtRtmpCtx->streams[pThis->m_iVideoOutIndex]->time_base, (AVRounding)(AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX));
					pkt.duration = ((pThis->m_pFmtRtmpCtx->streams[pThis->m_iVideoOutIndex]->time_base.den / pThis->m_pFmtRtmpCtx->streams[pThis->m_iVideoOutIndex]->time_base.num) / ENCODE_FPS);//pkt.pts - cur_pts_v;//
					pkt.pos = -1;
...
int CLS_DlgStreamPusher::OpenRtmpUrl()
{
	...
	if (NULL != m_pFmtVideoCtx){
		...
		m_pStreamInfo->m_pVideoFifo = av_fifo_alloc(3 * avpicture_get_size(AV_PIX_FMT_YUV420P, pVideoStream->codec->width, pVideoStream->codec->height));
	}
