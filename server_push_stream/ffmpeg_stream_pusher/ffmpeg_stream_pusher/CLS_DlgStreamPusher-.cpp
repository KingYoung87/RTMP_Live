...
void CLS_DlgStreamPusher::InitDlgItem()
{
	...
	m_edtPusherAddr.SetWindowText("rtmp://123.59.87.24:1935/hls/live_stream");
...
int push_thr(LPVOID lpParam)
{
	...
	while (1){
		...
		if (av_compare_ts(cur_pts_v, pThis->m_pFmtRtmpCtx->streams[pThis->m_iVideoOutIndex]->time_base,
			...
			in_stream = pThis->m_pFmtVideoCtx->streams[0];
			out_stream = pThis->m_pFmtRtmpCtx->streams[pThis->m_iVideoOutIndex];
			...
			if (av_fifo_size(pStrctStreamInfo->m_pVideoFifo) >= size){
				...
				//pts = n * (（1 / timbase）/ fps);
				picture->pts = frame_video_index * ((pThis->m_pFmtVideoCtx->streams[0]->time_base.den / pThis->m_pFmtVideoCtx->streams[0]->time_base.num) / 15);

				int got_picture = 0;
				iRet = avcodec_encode_video2(pThis->m_pFmtRtmpCtx->streams[pThis->m_iVideoOutIndex]->codec, &pkt, picture, &got_picture);
				if (iRet < 0 || got_picture != 1){
					//编码错误,不理会此帧
					av_free_packet(&pkt);
					continue;
				}
				...
				//Parameters  
				pkt.pts = (double)(frame_video_index*calc_duration) / (double)(av_q2d(time_base1)*AV_TIME_BASE);
				pkt.dts = pkt.pts;
				...
				(pThis->m_pFmtRtmpCtx->streams[pThis->m_iAudioOutIndex]->codec->frame_size > 0 ? pThis->m_pFmtRtmpCtx->streams[pThis->m_iAudioOutIndex]->codec->frame_size : 1024))
			{
				...
				in_stream = pThis->m_pFmtAudioCtx->streams[0];
				out_stream = pThis->m_pFmtRtmpCtx->streams[pThis->m_iAudioOutIndex];
				...
				if (avcodec_encode_audio2(pThis->m_pFmtRtmpCtx->streams[pThis->m_iAudioOutIndex]->codec, &pkt, frame, &got_picture) < 0){
					TRACE("can not decoder a frame");
					av_frame_free(&frame);
					av_free_packet(&pkt);
					continue;
				}
				if (NULL == pkt.data || got_picture != 1){
					TRACE("NULL == pkt.data");
					av_frame_free(&frame);
					av_free_packet(&pkt);
					continue;
				...
				int64_t calc_duration = (double)AV_TIME_BASE / av_q2d(in_stream->r_frame_rate);
				//Parameters  
				pkt.pts = (double)(frame_audio_index*calc_duration) / (double)(av_q2d(time_base1)*AV_TIME_BASE);
				pkt.dts = pkt.pts;
				pkt.duration = (double)calc_duration / (double)(av_q2d(time_base1)*AV_TIME_BASE);
				pkt.stream_index = pThis->m_iAudioOutIndex;
				frame_audio_index++;
				cur_pts_a = pkt.pts;
