	...
	while (1){
		...
		if (av_compare_ts(cur_pts_v, pThis->m_pFmtRtmpCtx->streams[pThis->m_iVideoOutIndex]->time_base,
				...
				(pThis->m_pFmtRtmpCtx->streams[pThis->m_iAudioOutIndex]->codec->frame_size > 0 ? pThis->m_pFmtRtmpCtx->streams[pThis->m_iAudioOutIndex]->codec->frame_size : 1024))
			{
				...
				avcodec_fill_audio_frame(frame, pAudioStream->codec->channels, pAudioStream->codec->sample_fmt, dst_samples_data[0], dst_samples_size, 0);
				...
				if (got_picture == 1){
					...
					pkt_out.pts = av_rescale_q(pkt_out.pts, pAudioStream->codec->time_base, pAudioStream->time_base);//  
					pkt_out.dts = av_rescale_q(pkt_out.dts, pAudioStream->codec->time_base, pAudioStream->time_base);//
...
int CLS_DlgStreamPusher::OpenRtmpUrl()
{
	...
	if (NULL != m_pFmtAudioCtx){
		...
		src_nb_samples = pAudioStream->codec->codec->capabilities & CODEC_CAP_VARIABLE_FRAME_SIZE ?
			10000 : pAudioStream->codec->frame_size;
		av_samples_alloc_array_and_samples(&src_samples_data, &src_samples_linesize, pAudioStream->codec->channels,
			src_nb_samples, AV_SAMPLE_FMT_S16, 0);
		max_dst_nb_samples = src_nb_samples;

		dst_samples_data[0] = src_samples_data[0];
		dst_samples_size = av_samples_get_buffer_size(NULL, pAudioStream->codec->channels, max_dst_nb_samples,
			pAudioStream->codec->sample_fmt, 0);
